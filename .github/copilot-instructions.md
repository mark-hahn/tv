...
#dev folder
- testing is being done now so remote folder is /root/dev/apps/tv-dev
...
- remote /root/dev/apps/tv-dev/ is not a repo or worktree, it is just a raw directory that pm2 uses.
- every path  that starts with /root/dev/apps/tv-dev is on the remote server.   
