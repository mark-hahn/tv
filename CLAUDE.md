...
## Never Run Unilog Reconciliation except after changing client source files
...
- Do not run `unilog/run-reconcile.js` except after changing client source files, do not edit `unilog/reconcile-cache.json`, and do not write to `unilog.sqlite` / the unilog DB. The `./srvr` deploy/release flow owns reconciliation, cache updates, ids, and DB metadata.
