...
function saveReelShows() {
  if (!reelShowsLoaded) {
    ...
    console.error(
      `saveReelShows aborted: Reel shows not loaded cleanly at startup.`,
    );
    return;
  }
...
function saveResultTitles() {
  if (!resultTitlesLoaded) {
    console.error(
      `saveResultTitles aborted: Titles not loaded cleanly at startup.`,
    );
    return;
  }
...
function appendResultTitle(entry) {
  ...
  if (newTitle) {
    resultTitles = resultTitles.filter(
      (existing) => parseResultTitle(existing) !== newTitle,
    );
  }
...
function shouldPersistResultEntry(entry) {
  ...
  return (
    !s.toLowerCase().startsWith("error|") && !s.toLowerCase().startsWith("msg|")
  );
}
...
try {
  ...
  console.error(
    "CRITICAL: Failed to load reel-shows.json on startup.",
    e.message,
  );
...
try {
  ...
  console.error(
    "CRITICAL: Failed to load reelgood-titles.json on startup.",
    e.message,
  );
...
function checkReel() {
  ...
  const seenTitles = new Set(
    resultTitles.map(parseResultTitle).filter(Boolean),
  );
...
export async function getReel() {
  try {
    if (!homeHtml) {
      logToFile(
        "getReel failed: homeHtml is null/empty. Call startReel first.",
      );
    ...
    const haveItSet = new Set(
      (Array.isArray(showTitles) ? showTitles : []).map((s) =>
