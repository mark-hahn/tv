...
async function main() {
  ...
  if (!targetUrl || !String(targetUrl).startsWith("http")) {
    fail(
      'Missing URL arg. Example: node torrents/test/curl-test.js "https://www.torrentleech.org/download/<fid>/<name>.torrent"',
    );
    return;
  }
  ...
  if (!reqBrowserPath) {
    fail(
      `Missing curl profile file. Tried:\n- ${REQ_BROWSER_CANDIDATES.join("\n- ")}`,
    );
    return;
  }
  ...
  if (!res.ok) {
    console.error("[curl-test] curl failed", {
      code: res.code,
      error: res.error,
    });
    if (res.stderr)
      console.error("[curl-test] curl stderr", res.stderr.slice(0, 4000));
    process.exitCode = 1;
    return;
  }
  ...
  if (firstByte !== "d".charCodeAt(0)) {
    console.warn(
      '[curl-test] warning: output does not look like bencoded torrent (first byte != "d")',
    );
  }
}
