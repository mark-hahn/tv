...
function main() {
  ...
  if (fs.existsSync(showsPath)) {
    try {
      ...
      if (!shows || typeof shows !== "object" || Array.isArray(shows)) {
        ...
        console.log("[reelgood-validate] reel-shows.json ok", {
          keys: Object.keys(shows).length,
        });
      }
    } catch (e) {
  ...
  if (fs.existsSync(titlesPath)) {
    try {
      ...
      if (!Array.isArray(titles)) {
        ...
        const dups = [...titleCounts.entries()]
          .filter(([, c]) => c > 1)
          .sort((a, b) => b[1] - a[1]);
        console.log("[reelgood-validate] reelgood-titles.json ok", {
          entries: titles.length,
          uniqueTitles: titleCounts.size,
          dupTitles: dups.length,
        });
        if (dups.length) {
  ...
  if (fs.existsSync(callsPath)) {
    ...
    console.log("[reelgood-validate] calls.log present", {
      myPetAteWhatOccurrences: hits,
    });
  }
}
