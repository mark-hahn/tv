...
console.log = function (...args) {
  ...
  if (!SHOW_RAW) {
     ...
     if (msg.includes("Overlapping segments")) return;
     if (/^\[\d{1,2}:\d{2}:\d{2}\] [AB] \[/.test(msg)) return; // Matches [ts] A [ts], ...
     if (/[AB] \[\d+:\d{2}:\d{2}.\d+\],/.test(msg)) return; // Generic match for A/B segment lines
     if (/^Using [AB]$/.test(msg)) return;
  }

  // Suppress standalone blank lines
  if (!msg.trim()) return;

  // Prefix newline for specific messages
  let prefix = "";
  if (
    msg.includes("Configuration:") ||
    /Found \d+ video file\(s\) to process/.test(msg) ||
    msg.trim().startsWith("Wrote ")
  ) {
    prefix = "\n";
  }
  ...
  let suffix = "";
  if (msg.trim().startsWith("Wrote ")) {
    suffix = "\n";
  }

  // Strip leading newlines from the message itself
  let cleanMsg = msg.replace(/^\n+/, "");
  
  // Strip existing elapsed timestamp [00:00:00] if present at start (generated by internal ts())
  cleanMsg = cleanMsg.replace(/^\[\d{1,2}:\d{2}:\d{2}\] /, "");
...
async function processOneVideo(videoPath) {
  ...
  console.log(`Elapsed: ${String(eMin).padStart(2, '0')}:${String(eSec).padStart(2, '0')}\n`);
} catch (err) {
    console.error(
