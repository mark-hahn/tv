...
async function logApiNetInfoOnce(logger, startMs) {
  ...
  try {
    ...
    logger.log(`[${ts(startMs)}] API dns: api.openai.com -> ${formatAddrList(addrs)}`);
  } catch (e) {
...
async function callTranscribeOnce({ apiKey, uploadInfo, chunkDurationSec, signal, logger, startMs, timeoutMs, httpsAgent, modelId }) {
  ...
  // OpenAI transcriptions follow the OpenAI-style API shape: segments are only
...
async function callApi({ apiKey, uploadInfo, chunkDurationSec, signal, logger, startMs }) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    if (logger) {
      logger.log(
        `[${ts(startMs)}] API: attempt ${attempt}/${MAX_ATTEMPTS} url=${apiUrl} model=${OPENAI_MODEL} family=4 keepAlive=${keepAlive ? '1' : '0'} timeoutMs=${timeoutMs}`,
      );
    }
    ...
    try {
      reqStart = Date.now();
      return await callTranscribeOnce({ apiKey, uploadInfo, chunkDurationSec, signal, logger, startMs, timeoutMs, httpsAgent, modelId: OPENAI_MODEL });
    } catch (err) {
      if (signal?.aborted) throw new Error('killed');
...
function readApiKey() {
  const keyPath = path.join(getAsrSecretsDir(), 'openai-asr-key.txt');
  const apiKey = fs.readFileSync(keyPath, 'utf8').trim();
  if (!apiKey) throw new Error('Missing openai-asr-key.txt contents');
  return apiKey;
}
...
export async function runAsrJob(job, { signal, onProgress, logger } = {}) {
  ...
  if (logger) {
    ...
    logger.log(`   API Model:         ${OPENAI_MODEL}`);
    logger.log(`   API URL:           https://api.openai.com/v1/audio/transcriptions`);
