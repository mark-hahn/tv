...
export default {
  ...
  computed: {
    ...
    tvdbId() {
      ...
      if (!s) return "";
      const raw =
        s?.ProviderIds?.Tvdb ?? s?.TvdbId ?? s?.tvdbId ?? s?.tvdb_id ?? "";
  ...
  methods: {
    onTvdbDataReady(data) {
      try {
        ...
        const currentId =
          this.activeShow?.Id != null ? String(this.activeShow.Id) : "";
        const currentName =
          this.activeShow?.Name != null ? String(this.activeShow.Name) : "";
