<template>
  ...
  >
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              v-if="hasSearched &amp;&amp; !loading &amp;&amp; totalSubsCount &gt; 0"
              style="font-size: 12px; color: #666; font-weight: normal"
            >
              {{ validSubsCount }}/{{ totalSubsCount }}
            </div>
          </div>
          <div
            v-if="fileIdStatusText"
            style="
              position: absolute;
              left: 50%;
        ...
        >
          <div style="display: flex; gap: 8px; align-items: center">
            <button
              ...
              :style="getLibraryButtonStyle()"
            >
              Library
            </button>
            <button
              @click.stop="applyClick"
            ...
            <button
              ...
              :style="getModeButtonStyle('search')"
            >
              Search
            </button>
            <button
              @click.stop="selectMode('season')"
              :style="getModeButtonStyle('season')"
            >
              Season
            </button>
            <button
              @click.stop="selectMode('episode')"
              :style="getModeButtonStyle('episode')"
            >
              Episode
            </button>
            <button
              @click.stop="selectMode('files')"
      ...
      <div
        v-if="showApplyFailuresModal"
        @click.self.stop="closeApplyFailuresModal"
        style="
          position: fixed;
          inset: 0;
          z-index: 1000;
      ...
      >
        ...
        >
          ...
          >
            ...
            <button
              @click.stop="closeApplyFailuresModal"
              style="
                font-size: 12px;
                cursor: pointer;
                border-radius: 7px;
          ...
          ></div>
          <div
            v-if="!applyFailures || applyFailures.length === 0"
      ...
      <div
        id="subs-list"
        v-if="!loading"
        style="padding: 10px; font-size: 14px; line-height: 1.2"
      >
        <div
          v-if="!hasSearched &amp;&amp; items.length === 0 &amp;&amp; !error"
          style="text-align: center; color: #999; margin-top: 50px"
        >
          <div>Press search to load subtitles.</div>
        </div>
        <div
          v-else-if="hasSearched &amp;&amp; totalSubsCount === 0 &amp;&amp; !error"
          style="text-align: center; color: #999; margin-top: 50px"
        >
        ...
        >
          <div
            class="se-divider"
            v-if="shouldShowSeDivider(index)"
            style="
              text-align: center;
              color: #888;
              font-family: monospace;
          ...
          >
            <div
              v-if="shouldShowClickedCheckmark &amp;&amp; isClicked(item)"
              style="
                position: absolute;
                top: 8px;
                right: 8px;
            ...
            >
              ...
              >
                <template v-if="viewMode === 'files'">
                  ...
                  <div
                    :style="{
                      ...
                      fontWeight: 'bold',
                      marginLeft: '4px',
                      visibility: isAppliedFile(item) ? 'visible' : 'hidden',
                    }"
                  >
                    ✓
                  </div>
                ...
                <template v-else>
                  ...
                  <div
                    v-if="hasAppliedMark(item)"
                    style="color: #4caf50; font-size: 14px; font-weight: bold"
                  >
                    ✓
                  </div>
                </template>
...
export default {
  ...
  computed: {
    ...
    appliedFileIdBase32Set() {
      const out = new Set();
      const arr = Array.isArray(this._subFileIdsBase32)
  ...
  methods: {
    ...
    normalizeHeaderErrorText(msg) {
      ...
      if (!s) return "Error";
      if (/^error\s*:/i.test(s))
        return `Error: ${s.replace(/^error\s*:/i, "").trim()}`;
      return `Error: ${s}`;
    },
    ...
    async acceptFileIdSearch() {
      ...
      const needSearch =
        ...
        !(Array.isArray(this._validEntries) && this._validEntries.length);
      if (needSearch) {
      ...
      try {
        ...
        if (
          ...
          this.setHeaderStatusText(
            this.normalizeHeaderErrorText(res.error),
            2000,
          );
        } else {
        ...
        const msg = (() => {
          ...
          if (typeof e === "object" && typeof e.error === "string")
            return e.error;
    ...
    findFileByBase32(base32Upper) {
      const target = String(base32Upper || "")
        .trim()
        .toUpperCase();
    ...
    scrollCardIntoView(cardKey) {
      ...
      if (elTop < visibleTop) {
        ...
        nextTop = Math.max(
          0,
          elBottom - (scroller.clientHeight || 0) + padding,
        );
      }
      if (nextTop != null) {
    ...
    handleItemClick(_event, item) {
      ...
      if (this.viewMode === "episode") {
        ...
        this.selectedEpisode =
          item?.episode != null ? Number(item.episode) : null;
        if (this.selectedSeason != null && this.selectedEpisode != null) {
          this._selectedEpisodeBySeason[String(this.selectedSeason)] =
            this.selectedEpisode;
        }
      }
    ...
    getSelectedSeasonNums() {
      const out = [];
      for (const k of Array.isArray(this.selectedSeasonKeys)
        ? this.selectedSeasonKeys
        : []) {
    ...
    getSelectedEpisodePairs() {
      const out = [];
      for (const k of Array.isArray(this.selectedEpisodeKeys)
        ? this.selectedEpisodeKeys
        : []) {
    ...
    computeEpisodeCardsForSelectedSeasons(selectedSeasons, bySeason, mapObj) {
      ...
      for (const s of seasons) {
        ...
        for (const e of eps) {
          const fileCount =
            seasonMap && Array.isArray(seasonMap.get(e))
              ? seasonMap.get(e).length
              : 0;
    ...
    computeFileCardsForSelectedEpisodes(selectedPairs, bySeason) {
      ...
      fileCards.sort((a, b) => {
        const sA =
          a?.season != null ? Number(a.season) : Number.POSITIVE_INFINITY;
        const sB =
          b?.season != null ? Number(b.season) : Number.POSITIVE_INFINITY;
        if (sA !== sB) return sA - sB;
        const eA =
          a?.episode != null ? Number(a.episode) : Number.POSITIVE_INFINITY;
        const eB =
          b?.episode != null ? Number(b.episode) : Number.POSITIVE_INFINITY;
    ...
    pruneEpisodeSelectionsToSelectedSeasons() {
      ...
      for (const k of Array.isArray(this.selectedEpisodeKeys)
        ...
        if (Number.isFinite(season) && selectedSeasons.has(season))
          keep.push(k);
      }
      ...
      const mapObj =
        this._seriesMapObj && typeof this._seriesMapObj === "object"
          ? this._seriesMapObj
          : null;
      const episodeCards = this.computeEpisodeCardsForSelectedSeasons(
    ...
    pruneFileSelectionsToSelectedEpisodes() {
      ...
      const existing = Array.isArray(this.selectedFileKeys)
        ? this.selectedFileKeys
        : [];
    ...
    getSelectionKeys(mode) {
      if (mode === "season")
        return Array.isArray(this.selectedSeasonKeys)
          ? this.selectedSeasonKeys
          : [];
      if (mode === "episode")
        return Array.isArray(this.selectedEpisodeKeys)
          ? this.selectedEpisodeKeys
          : [];
      if (mode === "files")
        return Array.isArray(this.selectedFileKeys)
          ? this.selectedFileKeys
          : [];
      return [];
    },
    ...
    buildFileIdObjsPayload() {
      ...
      if (this.viewMode === "season") {
        ...
        return this.buildFileIdObjsFromValidEntries(
          validEntries.filter((v) => seasons.has(v?.season)),
        );
      }
      ...
      if (this.viewMode === "episode") {
        ...
        return this.buildFileIdObjsFromValidEntries(
          validEntries.filter((v) => pairs.has(`${v?.season}-${v?.episode}`)),
        );
      }
        ...
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(
            () =>
              reject(
                new Error(
                  `applySubFiles: timed out after ${timeoutMs / 1000}s`,
                ),
              ),
            timeoutMs,
          );
        ...
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(
            () =>
              reject(
                new Error(
                  `deleteSubFiles: timed out after ${timeoutMs / 1000}s`,
                ),
              ),
            timeoutMs,
          );
    ...
    parseSeasonEpisodeFromEntry(entry) {
      ...
      for (const c of candidates) {
        ...
        if (bestEpisode == null && se?.episode != null)
          bestEpisode = se.episode;
    ...
    buildCardItemFromResult(entry) {
      ...
      const release =
        entry?.attributes?.release != null
          ? String(entry.attributes.release)
          : "";
      let uploader =
        entry?.attributes?.uploader?.name != null
          ? String(entry.attributes.uploader.name)
          : "";
      if (uploader && uploader.trim().toLowerCase() === "anonymous")
        uploader = "";
    ...
    buildFileCardItem(validEntry, fileObj) {
      ...
      if (fileId == null || season == null || episode == null || !entry)
        return null;
    ...
    getSeDividerText(index) {
      ...
      if (!item) return "================";
      const sStr =
        item.season != null ? String(item.season).padStart(2, "0") : "??";
      const eStr =
        item.episode != null ? String(item.episode).padStart(2, "0") : "??";
    ...
    rebuildVisibleItems() {
      ...
      const pickFirstSeason = () => {
        ...
        if (this.selectedSeason != null) {
          if (mapSeasonNums.length && list.includes(this.selectedSeason))
            return this.selectedSeason;
          if (!mapSeasonNums.length && bySeason.has(this.selectedSeason))
            return this.selectedSeason;
        }
      ...
      const pickFirstEpisode = (season) => {
        ...
        if (mapEps.length) {
          if (
            this.selectedEpisode != null &&
            mapEps.includes(this.selectedEpisode)
          )
        ...
        if (
          this.selectedEpisode != null &&
          seasonMap &&
          seasonMap.has(this.selectedEpisode)
        )
          return this.selectedEpisode;
      ...
      if (this.viewMode === "season") {
        ...
        const seasonCards = seasons.map((s) => {
          const mapEps = this.getMapEpisodeNums(mapObj, s);
          const episodeCount = mapEps.length
            ? mapEps.length
            : Array.from((bySeason.get(s) || new Map()).keys()).length;
        ...
        const existing = Array.isArray(this.selectedSeasonKeys)
          ? this.selectedSeasonKeys
          : [];
        ...
        const primaryKey =
          ...
          allowed.has(this._lastClickedKeyByMode.season)
            ? this._lastClickedKeyByMode.season
      ...
      if (this.viewMode === "episode") {
        ...
        const seasonsToShow = selectedSeasons.length
          ? selectedSeasons
          : [pickFirstSeason()].filter((v) => v != null);
        const episodeCards = this.computeEpisodeCardsForSelectedSeasons(
          seasonsToShow,
          bySeason,
          mapObj,
        ...
        const existing = Array.isArray(this.selectedEpisodeKeys)
          ? this.selectedEpisodeKeys
          : [];
        const pruned = existing.filter((k) => allowed.has(k));
        if (pruned.length) {
        ...
        const primaryKey =
          ...
          allowed.has(this._lastClickedKeyByMode.episode)
            ? this._lastClickedKeyByMode.episode
      ...
      if (this.viewMode === "files") {
        ...
        if (!pairs.length) {
          const selectedSeasons = this.getSelectedSeasonNums();
          const s0 = selectedSeasons.length
            ? selectedSeasons[0]
            : pickFirstSeason();
        ...
        const primaryKey =
          ...
          allowed.has(this._lastClickedKeyByMode.files)
            ? this._lastClickedKeyByMode.files
            : this.selectedFileKeys[0] || "";
        this.selectedCardKey = primaryKey;
        const primaryItem = Array.isArray(fileCards)
          ? fileCards.find((it) => it.key === primaryKey)
          : null;
        if (primaryItem?.season != null)
          this.selectedSeason = Number(primaryItem.season);
        if (primaryItem?.episode != null)
          this.selectedEpisode = Number(primaryItem.episode);
        return;
      }
    ...
    async searchClick() {
      ...
      try {
        ...
        const first = await this.fetchSubsPageWithRetry(
          imdbIdDigits,
          1,
          season,
        );
        ...
        for (let p = 2; p <= pagesToLoad; p++) {
          try {
            const next = await this.fetchSubsPageWithRetry(
              imdbIdDigits,
              p,
              season,
            );
        ...
        let validOnly = sortable.filter(
          (x) => x.season != null && x.episode != null,
        );
