...
const getEpisodes = async (seasonId) => {
  ...
  const Episodes = [];
  const episodes = (await axios.get(urls.childrenUrl(cred, seasonId))).data
    .Items;
  for (let key in episodes) {
    ...
    const unaired = !!unairedObj[episodeNumber] && !watched && !haveFile;
...
const getSeasons = async (showId, showNumber) => {
  const Seasons = [];
  const seasons = (await axios.get(urls.childrenUrl(cred, showId))).data.Items;
  for (let key in seasons) {
    let season = seasons[key];
    const seasonNumber = +season.IndexNumber;
    Seasons[seasonNumber] = await getEpisodes(
      season.Id,
      showNumber,
      seasonNumber,
    );
...
const getGap = (seasons) => {
  ...
  for (let seasonIdx = 0; seasonIdx < seasons.length; seasonIdx++) {
    ...
    for (let epiIndex = 1; epiIndex < season.length; epiIndex++) {
      ...
      if (hadUnaired && !unaired) {
        console.log(
          `-- aired after unaired: ${showName}, ` + `S${seasonIdx}E${epiIndex}`,
        );
      ...
      if (watched || haveFile) missingEndFileCnt = -1;
        ...
        if (missingEndFileCnt == -1 && lastEpiWatched && !watched && !haveFile)
          missingEndFileCnt = 0;
        if (missingEndFileCnt != -1 && !watched && !haveFile)
          missingEndFileCnt++;
      }
      lastEpiWatched = watched;

      ///////// not watched at beginning of season /////////
      if (firstEpisodeInSeasonNotWatched && watched && !firstEpisodeInSeason) {
        console.log(
          `-- not watched at beginning: ` +
      ...
      if (hadWatched && !watched) hadNotWatched = true;
        ...
        console.log(
          `-- watched gap: ` + `${showName}, S${seasonIdx}E${epiIndex}`,
        );
      ...
      if (hadFile && !haveFile) hadNoFile = true;
        ...
        console.log(`-- file gap: ${showName}, ` + `S${seasonIdx}E${epiIndex}`);
  ...
  if (missingEndFileCnt > 1) {
    console.log(
      `-- ${missingEndFileCnt} end episodes missing: ` + `${showName}`,
    );
...
self.onmessage = async (event) => {
  ...
  console.log(`seasons-worker started, ${allShowsIdName.length} shows`);
