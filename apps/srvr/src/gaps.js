// The per-show gap check: missing files, watch gaps, resolution drops and
// stray episodes, all read off the record's episodeData and the disk.
import fs from "fs";
...
const getShowState = (showName, showMeta) => {
  ...
  try {
        ...
        if (haveFile) {
          ...
          if (res) {
            ...
            if (
              ...
              if (resDropIsNew) {
                unilog(2514, `resDrop set for ${showName} S${seasonNumber}E${episodeNumber}: ${res} < ${bestResSoFar}`);
              }
            }
    ...
    unilog(2510, `getShowState error for ${showName}:`, error.message);
    return null;
  }
  const strayOk = null;
...
export const gapCheckOne = async (showId, showName, tvdbRecord) => {
  ...
  if (!showState) {
    unilog(2509, `getShowState returned null for ${showName}`);
    return null;
  }
