 ...
 * simple-mode tabs — Info, Map, Actors, Reviews, Trailers — holding the same
 * content it shows, so all three uis agree on what a show looks like.
 *
 * A pane is told about the selected show whether or not it is on screen, and
 * loads whatever it needs only once it is shown: two of them fetch, and running
 * a fetch a card at a time while someone scrolls the list would be for nothing.
 */
