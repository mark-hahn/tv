...
All commands use the existing WebSocket protocol at `wss://hahnca.com/tv-series-srvr` with the message format:
```
{id}~~~{functionName}~~~{jsonParams}
```

Response format:
```
{id}~~~{status}~~~{jsonResult}
```
