...
export function setTorrents(id, param, resolve, reject) {
  try {
    console.log('[torrents] setTorrents called with param:', typeof param, param);
    
    const torrents = param?.torrents;
    
    if (!Array.isArray(torrents)) {
      console.log('[torrents] Error: torrents is not an array:', typeof torrents, torrents);
      reject([id, 'setTorrents: torrents must be an array']);
      return;
    }

    console.log(`[torrents] Received ${torrents.length} torrents from client`);
...
export function getTorrent(id, param, resolve, reject) {
  try {
    const torrentId = param.torrentId;
