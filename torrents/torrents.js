...
async function searchPrivateTrackers(showName, credentials = {}) {
  const browser = await firefox.launch({
    headless: false // Changed to visible to avoid Cloudflare detection
  });
...
async function searchIPTorrents(context, showName, auth) {
  ...
  try {
    ...
    console.log(`IPT Page title: "${pageTitle}"`);
    console.log(`IPT Page URL: ${pageUrl}`);
    
    // Save screenshot for debugging
    await page.screenshot({ path: './cookies/ipt-debug.png' });

    // Extract torrent URLs
    const torrents = await page.evaluate(() => {
      const links = document.querySelectorAll('a[href*="/details.php"]');
      console.log('Total links with /details.php:', links.length);
      
      // Debug: log all hrefs
      const allHrefs = Array.from(links).map(link => link.href);
      console.log('Sample hrefs:', allHrefs.slice(0, 5));
    ...
    console.log(`Extracted ${torrents.length} unique torrent URLs`);
...
async function searchTorrentLeech(context, showName, auth) {
  ...
  try {
    ...
    // Wait longer for Cloudflare to finish
    console.log('Waiting for Cloudflare...');
    await page.waitForTimeout(5000);
    
    // Try waiting for body content to change
    try {
      await page.waitForFunction(() => {
        const text = document.body?.textContent || '';
        return !text.includes('Verify you are human') && !text.includes('Just a moment');
      }, { timeout: 30000 }); // Increased to 30 seconds for manual verification
      console.log('Cloudflare verification passed');
      
      // Wait additional time for page to fully load after verification
      await page.waitForTimeout(3000);
    } catch (e) {
      console.log('Cloudflare timeout - proceeding anyway');
    }

    // Debug: Check what we got
    const pageTitle = await page.title();
    const pageUrl = page.url();
    console.log(`TL Page title: "${pageTitle}"`);
    console.log(`TL Page URL: ${pageUrl}`);
    ...
    fs.writeFileSync('./cookies/tl-response.txt', pageText);
    console.log(`TL Response length: ${pageText.length} chars`);
    console.log(`TL Response preview: ${pageText.substring(0, 200)}`);
